...
> **勘误注（2026-09-21）**：本节移除的是**实时价格抓取、配件价格目录与整装成本模型**（需网络、
> 第三方依赖、人工编造数据），**这些仍然不做**。后续规范
> [`2026-09-21-ammo-kill-cost-design.md`](2026-09-21-ammo-kill-cost-design.md) 以**手工维护的单发均价表**
> 重新引入「单次击杀弹药成本」这一展示列，不改变本规范任何战斗口径（TTK 公式、`E[N]`、射击间隔、分层）。
> 本规范第 2 节「无经济性权重」的表述依然成立——成本仅作展示，不参与排序。
